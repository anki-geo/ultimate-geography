...
def require_expected_ug_field_deltas(target: str, comparison: dict[str, object]) -> None:
    actual = field_delta_set(comparison)
    reviewed_flags = EXPECTED_REVIEWED_FLAG_SIMILARITY_DELTAS.get(target, frozenset())
    followups = frozenset(
        (guid, index, before, after)
        for row_target, guid, index, before, after in FOLLOWUP_TRANSLATION_FIELD_DELTAS
        if row_target == target
    )
    expected = reviewed_flags | followups
    require(
        actual == expected,
        f"{target}: UG field deltas differ: expected {expected!r}, got {actual!r}",
    )
...
def compare_ug_target(spec: TargetSpec, old_output: Path, new_output: Path) -> dict[str, object]:
    ...
    require_expected_ug_field_deltas(spec.target, notes)
    ...
    if EXPECTED_REVIEWED_FLAG_SIMILARITY_DELTAS.get(spec.target):
        classification += "; reviewed flag-similarity corrections"
    if any(row[0] == spec.target for row in FOLLOWUP_TRANSLATION_FIELD_DELTAS):
        classification += "; audited translation corrections"
...
def write_report(
    ...
    for result in ug_results:
        ...
        flag_count = len(EXPECTED_REVIEWED_FLAG_SIMILARITY_DELTAS.get(spec.target, ()))
        ...
        classified_deltas = []
        if flag_count:
            classified_deltas.append(f"{flag_count} reviewed flag-similarity correction tuple(s)")
        if followup_count:
            classified_deltas.append(f"{followup_count} exact audited translation correction tuple(s)")
        if classified_deltas:
            note_surface = "GUID/order/tags exact; " + " plus ".join(classified_deltas)
        surface_rows.append(
            [
                spec.target,
