...
Brew alpha.2, and writes a deterministic Markdown report. Unknown semantic
...
PINNED_BRAINBREW_REVISION = "f5f45a3d091985733d8979ef512411d4020bf941"
...
def compare_ug_target(spec: TargetSpec, old_output: Path, new_output: Path) -> dict[str, object]:
    ...
    require_blank_note_policy(new, (), f"{spec.target}: alpha.2 UG")
...
def compare_companion_target(
    ...
    require_blank_note_policy(new, (), f"{spec.target}: alpha.2 companion")
...
def compare_standalone_target(
    ...
    require_blank_note_policy(new, (), f"{spec.target}: alpha.2 standalone")
...
- Migration builder: `{new_command}` (`1.0.0-alpha.2`, immutable source revision `{PINNED_BRAINBREW_REVISION}`).
...
{markdown_table(['Representative target', 'Immutable baseline HTML', 'Current alpha.2 HTML'], hint_rows)}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="command prefix for alpha.2 (default: immutable pinned Nix revision)",
    )
